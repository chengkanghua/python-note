module Tpm {
    export class CardLogic {
        ...
        public changeSeat(seatID:number):UserPosition{
            var mySeatID: number = App.DataCenter.UserInfo.myUserInfo.seatID;
         ...
         * 获取动作列表
         */
        public getActListFromKey(actInfo: any) {
            ...
            for(var key in actInfo) {
                if (Number(key) != ACT_state.Act_Out) {
                    actList.push(Number(key));

                    var moFlag = (<GameScene>App.SceneManager.getScene(SceneConst.GameScene)).cardMod.getOwnMoFlag();
                    if (moFlag) {
                        if (Number(key) == ACT_state.Act_Ting || Number(key) == ACT_state.Act_BuGang || Number(key) == ACT_state.Act_AnGang) {
            ...
            for (var key in actInfo) {
                if (Number(key) == ACT_state.Act_DianHu || Number(key) == ACT_state.Act_Zimo) {
