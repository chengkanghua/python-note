...
def customer_add(request):
    ...
    form = CustomerForm(data=request.POST)     # 提交验证字段
    if form.is_valid():
        form.save()      # 对提交的数据保存到数据中
...
def customer_edit(request, cid):
    ...
    if request.method == 'GET':
        ...
        return render(request, 'customer_add.html', {'form': form})
    form = CustomerForm(data=request.POST, instance=obj)  # 验证传过来的地段合法性
    if form.is_valid():
        form.save()    # 保存到数据库
        return redirect('/customer/list/')
    return render(request, 'customer_edit.html', {'form': form}) # 校验失败
...
def customer_import(request):
    ...
    try:
        ...
        """
        print(customer_excel)  # customer_excel_tpl.xlsx
        print(type(customer_excel))  # <class 'django.core.files.uploadedfile.InMemoryUploadedFile'>
...
def customer_tpl(request):  # templates
    """
    下载批量导入Excel列表
    :param request:
    :return:
    ...
    print(mimetypes.guess_type(tpl_path))
    content_type = mimetypes.guess_type(tpl_path)[0]  # 文件类型
    print(content_type)
    response = FileResponse(open(tpl_path, mode='rb'), content_type=content_type)
    response['Content-Disposition'] = "attachment;filename=%s" % 'customer_excel_tpl.xlsx'    # 下载的文件名 customer_excel_tpl.xlsx
    return response
