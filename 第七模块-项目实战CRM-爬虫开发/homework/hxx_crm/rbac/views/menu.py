...
def menu_list(request):
    ...
    second_menu_exists = models.Permission.objects.filter(
        id=second_menu_id).exists()
...
def multi_permissions(request):
    ...
    for name, value in permission_dict.items():
        # {'name': 'rbac:role_list', 'url': '/rbac/role/list/'},
...
def distribute_permissions(request):
    ...
    # 所有二级菜单
    all_second_menu_list = models.Permission.objects.filter(
    ...
    # 所有三级菜单（不能做菜单的权限）
    all_permission_list = models.Permission.objects.filter(
