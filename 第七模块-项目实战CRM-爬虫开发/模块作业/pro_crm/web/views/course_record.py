...
class CourseRecordHandler(PermissionHandler,StarkHandler):
    ...
    def action_multi_init(self,request,*args,**kwargs):
        ...
        student_object_list = class_object.student_set.all() #班级的所有学生对象列表
        for course_record_id in course_record_id_list:
            ...
            if not course_record_object:
                continue
            # 为每个学生在该天创建考勤记录
            study_record_object_list = [models.StudyRecord(student_id=stu.id, course_record_id=course_record_id) for
                                        stu in student_object_list]
            # 一次将所有对象插入数据库,  batch_size=50每次操作50条记录
