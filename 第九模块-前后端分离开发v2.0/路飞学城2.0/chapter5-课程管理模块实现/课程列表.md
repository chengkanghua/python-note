...
courses/management/commands/example.py
...
```bash
(luffycity) kanghua@ubuntu:~/Desktop/luffycity$ cd luffycityapi/
(luffycity) kanghua@ubuntu:~/Desktop/luffycity/luffycityapi$ python manage.py example
添加测试数据
...
````python
#! /home/kanghua/.conda/envs/luffycity/bin/python
...
mkdir $1 && cd $1 && echo hello > index.html
```
...
------
AccessKey ID	LTAI5t6L931KAY1EMXVY8YWT

AccessKey Secret BpGI6eHkM2TTjf9ra09PruefLAhphn
```
...
if __name__ == '__main__':
    OSS_ACCESS_KEY_ID = "LTAI5t6L931KAY1EMXVY8YWT"
    OSS_ACCESS_KEY_SECRET = "BpGI6eHkM2TTjf9ra09PruefLAhphn"
    OSS_ENDPOINT = "oss-cn-hangzhou.aliyuncs.com"  # 访问域名, 根据服务器上的实际配置修改
    OSS_BUCKET_NAME = "luffycity1"  # oss 创建的 BUCKET 名称
...
服务端调整url路由，在获取课程分类时允许地址栏传递学习方向的ID进来。 courses/urls.py
...
服务端添加测试数据，`luffycityapi/scripts/test_data.sql`，代码： 粘贴到 msyql console 执行
