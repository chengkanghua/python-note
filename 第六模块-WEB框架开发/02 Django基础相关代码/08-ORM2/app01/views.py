...
def add(request):
    ...
    # book_obj.authors.add(1,2,3)  # 直接写id号也可以
...
def query(request):
    ...
    # print(book_obj.publishDate,book_obj.price)
    ...
    # ret = publish.book_set.all()   # 反向查询按表名小写_set.all()
    # print(ret)
    ...
    # select avg(price),max(price) from app01_book;
    ...
    #
    # Emp.objects.all().annotate(avg_salary=Avg("salary")) # 在单表分组下,按着主键进行group by是没有任何意义的.
    ...
    # ret = Book.objects.values("title").annotate(c=Count("authors__name"))
    # print(ret)
    ...
    # ret = Publish.objects.all().annotate(c=Count("book__title")) # [obj,obj..]
    ...
    # ret = Book.objects.filter(title__startswith="py").annotate(authors_num=Count("authors__name")).values("title","authors_num")
    # print(ret)
    ...
    # +显示书名
    # ret = Publish.objects.annotate(min_price=Min("book__price")).values_list("name","book__title","min_price")

    # (2) 统计每一本书的作者个数
    # ret = Book.objects.annotate(authorsNum=Count("authors__name")).values("title","authorsNum")

    # (5) 根据一本图书作者数量的多少对查询集 `QuerySet`进行排序:
    # ret = Book.objects.annotate(sum_author=Count("authors")).values("title","sum_author").order_by("sum_author")
    # print(ret)
    ...
    # 查询评论数大于阅读数的书籍
    # ret = Book.objects.filter(comment_sum__gt=F("read_sum"))
    # print(ret)
    # Book书籍所有价格+1
    # Book.objects.all().update(price=F('price')+1)

    # 查询书籍名称是 红楼梦 或者 价格 102的书籍
    from django.db.models import F,Q
    # ret = Book.objects.filter(title="红楼梦", price=102)  # 这个是 且的关系
    # ret = Book.objects.filter(Q(title="红楼梦") | Q(price=102)) # | 或 & 且 还可以加()嵌套

    # 不等于红楼梦的书籍 并且 价格不等于101
    # ret = Book.objects.filter(~Q(title="红楼梦") & ~Q(price=101))
    #
    # print(ret)
